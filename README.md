...
* `smfs/estimate_lambda_poisson.py`: Estimates a global xi parameter using a Poisson model with site-specific sampled mutation rates. It separates segregating and non-segregating sites, minimizes the negative log-likelihood for xi, writes the xi estimate, and computes the expected number of sites with each possible number of unique mutations.

* `smfs/estimate_lambda_negative_binomial.py`: Estimates xi while accounting for uncertainty in mutation-rate/theta estimates using a negative-binomial marginal model. It uses per-site mean and standard deviation columns, estimates xi by likelihood optimization, and writes expected counts of sites with each number of unique mutations.
